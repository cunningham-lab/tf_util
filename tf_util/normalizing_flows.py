...
def get_flow_num_params(flow_class, D):
    if (flow_class == PlanarFlowLayer):
        return 2*D + 1
    elif (flow_class == ElemMultLayer):
        return D
    else:
...
class Layer:
    ...
    # Attributes
        self.params (tf.tensor): [K, self.num_params] tensor containing K parameterizations
                                 of the layer. 
...
class PlanarFlowLayer(Layer):
    def __init__(self, params, inputs):
        ...
        wdotu = tf.matmul(tf.expand_dims(self.w, 1), tf.expand_dims(self._u, 2))[:,:,0]
    ...
    def forward_and_jacobian(self,):
        ...
        # compute output
        z = z + tf.matmul(h, tf.expand_dims(self.u, 1)) # [K,n,D]
