...
def aug_lag_cost(
    H, T_x_mu_centered, Lambda, c, all_params, num_suff_stats, entropy=True, I_x=None
):
...
def test_get_array_str():
    ...
    b = np.array([1.155e-10, -3.222e10])
    assert get_array_str(a) == "1.00E+00_2.00E+00_3.00E+00_4.00E+00"
    assert get_array_str(b) == "1.16E-10_-3.22E+10"
    return None


def test_get_initdir():
    Ds = [1, 2, 10]
    rs = 1
    for D in Ds:
        arch_dict = {
        ...
        mus = [20 * np.ones(D), np.random.normal(0, 1, (D,))]
        sigmas = [np.ones(D), np.abs(np.random.normal(0, 1, (D,)))]
        _as = [np.random.normal(0, 1, (D,)), np.random.normal(0, 1, (D,))]
        bs = [np.random.normal(10, 1, (D,)), np.random.normal(10, 1, (D,))]
        ...
        for i in range(2):
            ...
            a_i = _as[i]
            b_i = bs[i]
            mu_str = "mu=%s" % get_array_str(mu_i)
            sigma_str = "sigma=%s" % get_array_str(sigma_i)
            _initdir_nb = "data/inits/%s/%s_%s_rs=%d/" % (archstring, mu_str, sigma_str, rs)
            initdir_nb = get_initdir(
                arch_dict, rs, init_type="gauss", mu=mu_i, sigma=sigma_i
            )
            assert initdir_nb == _initdir_nb

            a_str = "a=%s" % get_array_str(a_i)
            b_str = "b=%s" % get_array_str(b_i)
            _initdir_b = "data/inits/%s/%s_%s_%s_%s_rs=%d/" % (
                archstring,
                mu_str,
                sigma_str,
                a_str,
            ...
            initdir_b = get_initdir(
                arch_dict, rs, init_type="gauss", mu=mu_i, sigma=sigma_i, a=a_i, b=b_i
            )
            assert initdir_b == _initdir_b
...
if __name__ == "__main__":
    np.random.seed(0)
    test_fisher_information_matrix()
    test_get_array_str()
    test_get_initdir()
