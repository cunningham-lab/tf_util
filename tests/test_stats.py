...
import numpy as np
import scipy.stats
from tf_util.stat_util import approx_equal, get_dist_str, \
                              get_sampler_func, get_density_func
...
def test_get_dist_str():
    ...
    b_vec = a_vec + (np.abs(np.random.normal(0.0, 100.0, (n,))) +.001)
    ...
    for D in Ds:
        ...
        for k in range(K):
            ...
            for i in range(n):
                ...
                p_z_true = (1.0 / np.sqrt(np.linalg.det(2*np.pi*Sigma)) * \
                           np.exp(-0.5* np.dot(np.transpose(diff), np.dot(np.linalg.inv(Sigma), diff))))
...
def test_isotropic_truncated_normal():
    for D in Ds:
        ...
        for k in range(K):
            mu = np.abs(np.random.normal(0.0, 1.0, (D,))) + 0.001
            scale = np.abs(np.random.normal(0.0, 1.0, (1,))) + 0.001
            iso_tn_dist = {"family":"isotropic_truncated_normal", "mu":mu, "scale":scale}
...
def test_isotropic_inv_wishart():
    for D in Ds:
        for k in range(K):
            ...
            dist_true = scipy.stats.invwishart(df=df_fac*D, scale=df_fac*D*np.eye(D))
